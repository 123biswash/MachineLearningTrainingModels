# -*- coding: utf-8 -*-
"""Copy of Lab 6: Creating Validation Data.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1hfbyWCxndGsMmRXCjSWrDNl7xfYYMbl1
...
compute_loss(linear_regressor, test_input_fn, test_examples[LABEL])

compute_loss(linear_regressor, train_input_fn, training_examples[LABEL])

compute_loss(linear_regressor, eval_input_fn, validation_examples[LABEL])
...
A) Do you see any other differences in the distributions of features or targets
   ...
   Answer:  There is the difference between the latitude features of the 
   training and validation data. While looking at the min and max of the 
   validadation data and training data, they are quite separated.
   
  
B) Why is this happening?
  Answer:  This is happening becuase the validataion data is not takens as 
  a random from the overall data pool. I think the data is sorted in the 
  order of latitudes and longitudes. Training data uses 14000(almost 80% of 
  dataframe) and Validation data uses 3000(alomost 20 % of dataframe). 
...
C) How is this problem reflected when you look at the relationship between the
   ...
   Answer: The difference in RMSE between validation 
    data and training data is large. The difference between error in test data 
    and validation data is also large
...
training_examples = prepare_features(california_housing_dataframe.tail(14000))
validation_examples = prepare_features(california_housing_dataframe.head(3000))
...
ANSWER: 
New training and validation errors:

Training Loss (RMSE): 117.52
Validation Loss (RMSE): 106.20

Old training and validation errors:
Training: 134.47
Validation: 136.85
"""
