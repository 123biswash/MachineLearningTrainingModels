# -*- coding: utf-8 -*-
"""Copy of Lab 10: Regularization to Reduce Overfitting and Model Size.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1UW3LEyjQLyXQKDSFEMew26J8_HoHsgd9
...
"""# Lab 10: Regularization to Reduce Overfitting and Model Size
**Learning Objectives:**
...
***As in Lab 9, you will need to go to previous labs to make use of functions provided in those labs. It is very important you start this lab early so you have plenty of time to get help if needed! This is especially important if you did not complete Lab 9.***
...
"""### Modifications Introduced for Lab 10

As in Lab 9, you are going to copy a lot of your code from previous labs to complete this lab. Some of the key things you need to add here are:

* Defining the input functions.
* Defining and using the methods for feature scaling and also to compute quantiles to bucketize any desired features.
* Defining and using the methods to prepare the features.
* Generating the training and validation sets, and loading the test data.
* Defining the method to compute the loss, plot the learning curve, and train the model.

You should be able to find and use functions from previous labs with little or no modifications.

There are a few key changes that we are going to make and will provide some code snippets to help you make the needed changes.  We are continuing to provide less code so that when you go to apply what you are learning to new data sets you will know how to do everything needed to train and evaluate a model.  The key changes introduced in this lab are:
* Modifying the function to plot an ROC curve to take a set of curves so that they can all be plotted together providing a nice way to compare the performance of different models.
* Replace SGD with a different optimizier, FTRL, that among other things provides an easy way to introduce L2 and L1 regularization.
...
"""Note that we can re-write `make_roc_curve` to just call this function.  It would also work to just copy the previous definition of `make_roc_curve`."""
...
You might have noticed that we used a different optimizer than SGD. High dimensional linear models benefit from using a variant of gradient-based optimization called FTRL. This algorithm has the benefit of scaling the learning rate differently for different coefficients, which can be useful if some features rarely take non-zero values (it also is well suited to support L1-regularization). We can apply FTRL using the [FtrlOptimizer](https://www.tensorflow.org/api_docs/python/tf/train/FtrlOptimizer).

NOTE: This assumes you have defined the method `construct_feature_columns` as done in prior labs.
"""
...
def construct_feature_columns():
  ...
  # You need to write this.  I recommend you use what you had in Lab 9.
  # We have provided a skeleton method that just uses a single real-valued
  # feature.
...
"""## Task 1: Train a model without any regularization (1 point)

Just to be sure you have everything set up, train a model without any regularization.  This will be just like what you did in Lab 9 except using FTRL.  This part is primarily just to be sure you've copied over all the needed code. Since you are using a different optimizer, you will need to adjust the learning rate and the number of steps to train.

**As discussed in the slides and in Lab 9, because the log loss has a gradient that goes to infinity as your prediction approaches the target value, when training a logistic regression model with a lot of features and thus the possibility to overfit the training data, you can get a gradient that is so large that your model overflows. If you see an error indicating that you divided by zero or a loss of NaN, then most likely this situation has occured. The way to address this problem is to introduce L2 regularization, which you will do in Task 2.**
...
# You will need to adjust the learning rate and number of steps as in the past
LEARNING_RATE = 2.0
L1_REGULARIZATION_STRENGTH = 0.0
L2_REGULARIZATION_STRENGTH = 0.0
STEPS = 850
...
"""## Task 2 - Train a Linear Classifier with L2-Regularization (1 point)

Add L2-regularization with a large enough value to control the overfitting without making it large enough that the performance (metrics such as loss and AUC on the validation data) are affected.

You might need to adjust the learning rate and the number of steps that you train
"""
...
"""## Task 3 - Introduce L1 Regularization to Reduce the Model Size (2.5 points)

Next we explore the trade-offs between performance and model size by training models with different levels of L1-regularization. We've set up code blocks for you to plot ROC curves from Task 2 along with the ROC curves from models trained with increasing levels of L1-regularization. If you'd like to try additional levels of L1-regularization, feel free to add additional code blocks to do that. You can leave the same value of L2-regularization that you selected in Task 2.
...
"""### Creating ROC curves to compare the models

Next we provide you with code to generate the ROC curves. We start by setting up the lists of
validation probabilities and model names for the models you'd like to display. You can reset these
at any point by just executing this cell again.
"""

validation_probs = []
model_names = []

"""Running the code block below on a **trained** model will add its validation probabilities needed to display its ROC curve along with adding a model name for the legend that contains the L1-regularization used and the model size. Let's start by including the model trained in Task 2 without any L1-regularization."""
...
model_name = "L1="+str(L1_REGULARIZATION_STRENGTH)+", size"+str(size)
model_names.append(model_name)

"""### Adding L1-regularization to reduce the model size

Now introduce L1-regularization to bring the model size down to around 50.
"""

# Don't forget to fill in the value you used for L2 regularization strength from
# Task 2. You will most likely need to adjust the learning rate and number of
# steps to train once you've modified the L1 regularization strength.
LEARNING_RATE = 2.0
L1_REGULARIZATION_STRENGTH = 3
...
"""Once you've found the hyperparameter values that allow you to train a good model of the
desired size, you can run the below code block to add it to the plot of ROC curves.
"""
...
"""### Further increase L1-regularization to get a smaller model

Finally, pick a level of L1-regularization to bring the model size down to around 20 or less.
"""
...
"""## Task 4 - Look at features with non-zero weights (0.5 points)

Let's look at the non-zero weights in the smallest model you trained (the one with the highest amount of L1-regularization).
"""
...
for variable in variables:
  if not any(x in variable 
    ...
    weights =  linear_classifier.get_variable_value(variable)
    if np.count_nonzero(weights) > 0:
      print variable, weights[np.nonzero(weights)]

'''
Briefly discuss what you are seeing when looking at the ROC curves and in
looking at the non-zero weights.

The Model size is decreasing as L1 regularization strength increases. 
The AUC is decreasing as model size increases, which means the accuracy decreases as model size increases. 
The non-zero weights are as follows:
linear/age_bucketized/weights [-0.94408625 -0.31514403  0.24568371  0.12257534]
linear/age_bucketized_X_gender/weights [ 0.01718995  0.30029246 -0.0463329   0.3314684  -0.416155   -0.14307694]
linear/capital_gain_bucketized_X_capital_gain_bucketized/weights [ 0.08119639 -0.91291904]
linear/education/weights [ 0.4319487   0.25639024 -0.5356003  -0.15643992  0.00640854]
linear/gender/weights [-0.7181912   0.18333592]
